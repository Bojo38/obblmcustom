        ...
        if ($type == SG_T_LEAGUE) {
            ...
            */
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                array_push($queries, "SUM(IF($range, 1, 0)) AS 'Matchs_m$i'"); 
            ...
            $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches";
            $result = mysql_query($query);
            $row    = mysql_fetch_assoc($result);
            
            $lengends = array('Matchs' => 'royalblue');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Matchs joués", "Mois", "Matchs", $opts));
            ...
            */
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                array_push($queries, 
                    ...
                    (
                    SELECT team1_id AS 'tid', date_played FROM ".$db_prefix."matches WHERE $range 
                    UNION 
                    SELECT team2_id AS 'tid', date_played FROM ".$db_prefix."matches WHERE $range
                    ) AS tmptbl) AS 'Equipes actives_m$i'"
                ); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'\n");
            }
            ...
            $lengends = array('Equipes actives' => 'olivedrab');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Equipes actives", "Mois", "Equipes", $opts));
            ...
            */
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                array_push($queries, 
                    ...
                    (
                    SELECT owned_by_coach_id AS 'cid', date_played FROM ".$db_prefix."matches, ".$db_prefix."teams WHERE team1_id = team_id AND $range 
                    UNION 
                    SELECT owned_by_coach_id AS 'cid', date_played FROM ".$db_prefix."matches, ".$db_prefix."teams WHERE team2_id = team_id AND $range
                    ) AS tmptbl) AS 'Coachs Actifs_m$i'"
                ); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'\n");
            }
            ...
            $lengends = array('Coachs Actifs' => 'darkorange1');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Coachs Actifs", "Mois", "Coachs", $opts));            
            ...
            */
            ...
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Matchs jous", "Jours", "Matchs", $opts));
            ...
            */
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                # m$i = minus/negative $i months from present month.
                array_push($queries, "SUM(IF($range, cp, 0))     AS 'Passes_m$i'"); 
                array_push($queries, "SUM(IF($range, td, 0))     AS 'Touchdowns_m$i'"); 
#                array_push($queries, "SUM(IF($range, intcpt, 0)) AS 'int_m$i'"); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
            }

            $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches, ".$db_prefix."match_data WHERE f_match_id = match_id";
            $result = mysql_query($query);
            $row    = mysql_fetch_assoc($result);
            
            $lengends = array('Passes' => 'seagreen', 'Touchdowns' => 'indianred');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Historique des passes et touchdowns", "Mois", "Nombre", $opts));
            ...
            */
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                # m$i = minus/negative $i months from present month.
                array_push($queries, "SUM(IF($range, bh+si+ki, 0)) AS 'Sorties_m$i'"); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
            }
            ...
            $lengends = array('Sorties' => 'firebrick1');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Historique des sorties", "Mois", "Nombre", $opts));
...
#            ) AS starsTbl";
#            $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches, $tableMercs, $tableStars WHERE mercsTbl.f_match_id = matches.match_id AND starsTbl.f_match_id = matches.match_id";
#            $result = mysql_query($query);
#            $row    = mysql_fetch_assoc($result);
#            
...
#            $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches, match_data WHERE f_match_id = match_id";
#            $result = mysql_query($query);
#            $row    = mysql_fetch_assoc($result);
#            
...
#            array_push($graphs, $graph);
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                # m$i = minus/negative $i months from present month.
                array_push($queries, "SUM(IF($range, bh, 0)) AS 'Légères_m$i'"); 
                array_push($queries, "SUM(IF($range, si, 0)) AS 'Graves_m$i'"); 
                array_push($queries, "SUM(IF($range, ki, 0)) AS 'Morts_m$i'"); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
            }

            $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches, ".$db_prefix."match_data WHERE f_match_id = match_id AND $where";
            $result = mysql_query($query);
            $row    = mysql_fetch_assoc($result);
            
            $lengends = array('Légères' => 'forestgreen', 'Graves' => 'firebrick', 'Morts' => 'blue');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Historique des Bléssures légères, graves et tués", "Mois", "Nombre", $opts));
...
#                array_push($queries, "SUM(IF($range, intcpt, 0)) AS 'int_m$i'"); 
            ...
            $lengends = array('Passes' => 'forestgreen', 'Touchdowns' => 'firebrick');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Historique des passes et des touchdowns", "Mois", "Nombre", $opts));
            ...
            foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                ...
                # m$i = minus/negative $i months from present month.
                array_push($queries, "SUM(IF($range, IF(inj = ".MNG.",  1, 0)+IF(agn1 = ".MNG.",  1, 0)+IF(agn2 = ".MNG.",  1, 0), 0)) AS 'Rate un match_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".NI.",   1, 0)+IF(agn1 = ".NI.",   1, 0)+IF(agn2 = ".NI.",   1, 0), 0)) AS 'Persisitante_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".MA.",   1, 0)+IF(agn1 = ".MA.",   1, 0)+IF(agn2 = ".MA.",   1, 0), 0)) AS 'Mouvement_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".AV.",   1, 0)+IF(agn1 = ".AV.",   1, 0)+IF(agn2 = ".AV.",   1, 0), 0)) AS 'Armure_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".AG.",   1, 0)+IF(agn1 = ".AG.",   1, 0)+IF(agn2 = ".AG.",   1, 0), 0)) AS 'Agilité_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".ST.",   1, 0)+IF(agn1 = ".ST.",   1, 0)+IF(agn2 = ".ST.",   1, 0), 0)) AS 'Force_m$i'"); 
                array_push($queries, "SUM(IF($range, IF(inj = ".DEAD.", 1, 0)+IF(agn1 = ".DEAD.", 1, 0)+IF(agn2 = ".DEAD.", 1, 0), 0)) AS 'Mort_m$i'"); 
                array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
            }
            ...
            $lengends = array('Rate un match' => 'green', 'Persisitante' => 'red', 'Mouvement' => 'blue', 'Armure' => 'aqua', 'Agilité' => 'brown', 'Force' => 'purple', 'Mort' => 'slategray');
            list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
            array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Types de blessures subies", "Mois", "Nombre", $opts));
            ...
            if ($type == SG_T_TEAM) {
                ...
                foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                    ...
                    # m$i = minus/negative $i months from present month.
                    array_push($queries, "SUM(IF((team1_score > team2_score AND team1_id = $o->team_id OR team1_score < team2_score AND team2_id = $o->team_id) AND $range, 1, 0)) AS 'Victoires_m$i'"); 
                    array_push($queries, "SUM(IF((team1_score < team2_score AND team1_id = $o->team_id OR team1_score > team2_score AND team2_id = $o->team_id) AND $range, 1, 0)) AS 'Défaites_m$i'");
                    array_push($queries, "SUM(IF(team1_score = team2_score AND $range, 1, 0)) AS 'Nuls_m$i'");
                    array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                    array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
                }

                $query  = "SELECT ".implode(', ', $queries)." FROM ".$db_prefix."matches WHERE team1_id = $o->team_id OR team2_id = $o->team_id";
                $result = mysql_query($query);
                $row    = mysql_fetch_assoc($result);
                
                $lengends = array('Victoires' => 'forestgreen', 'Défaites' => 'firebrick', 'Nuls' => 'blue');
                list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
                array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Historique des victoires/nul/défaites", "Mois", "Matches", $opts));
                ...
                */
                ...
                foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                    ...
                    # m$i = minus/negative $i months from present month.
                    array_push($queries, "AVG(IF($range, IF(team1_id = $o->team_id, income1, income2)/1000, NULL)) AS 'Changement_m$i'"); 
                    array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                    array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
                }
                ...
                $lengends = array('Changement' => 'darkolivegreen4');
                list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
                array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Changement de trésorerie par match (moyenne)", "Mois", "Changement", array_merge($opts, array('scale' => 'textlin'))));
                ...
                */
                ...
                foreach (range(0, SG_MULTIBAR_HIST_LENGTH) as $i) {
                    ...
                    # m$i = minus/negative $i months from present month.
                    array_push($queries, "SUM(IF($range, IF(team1_id = $o->team_id, smp1, smp2), 0))     AS 'Sportivité_m$i'"); 
                    array_push($queries, "YEAR(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'yr_m$i'");
                    array_push($queries, "MONTH(SUBDATE(DATE(NOW()), INTERVAL $i MONTH)) AS 'mn_m$i'");
                }
                ...
                $lengends = array('Sportivité' => 'blue');
                list($datasets, $labels) = SGraph::mbarsInputFormatter($lengends, $row);
                array_push($graphs, SGraph::mbars($datasets, $labels, $lengends, "Points de sportivité accordés", "Mois", "Points", $opts));
            }
        }
