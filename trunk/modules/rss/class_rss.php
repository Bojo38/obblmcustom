...
class RSSfeed implements ModuleInterface
{
	...
	public function generateNewsRssFeed() {
        ...
        foreach (array_slice($entries, 0, RSS_SIZE) as $item) {
            ...
            $el_item->appendChild($dom->createElement('title', htmlspecialchars($item->title, ENT_NOQUOTES, "ISO-8859-1")));
            $el_item->appendChild($dom->createElement('description', htmlspecialchars($item->desc, ENT_NOQUOTES, "ISO-8859-1")));
