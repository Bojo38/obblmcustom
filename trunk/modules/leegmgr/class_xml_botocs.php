...
class XML_BOTOCS implements ModuleInterface
{
    ...
    function __construct($team_id, $jm) {
        ...
        $team = new Team ( $_GET["teamid"] );
            ...
            $this->race = get_alt_col('races','name',$team->f_rname,'botocs_name');
            $this->name = $team->name;
    ...
    function createRoster() {
        ...
        $this->roster .= "<?xml-stylesheet type=\"text/xsl\" href=\"team.xsl\"?>\n";
        $this->roster .= "<team>\n";
    ...
    private static function random_team($coach_id) {
        ...
        $result = mysql_query( "SELECT team_id FROM ".$db_prefix."teams WHERE owned_by_coach_id = $coach_id AND name LIKE '%[P]' AND RETIRED = 0 ORDER BY name ASC" );
    ...
    public static function main($argv) {
        ...
        if ( isset($_GET["teamid"]) && $_GET["teamid"] )
        {
            ...
            if ( !isset($_GET["cy"]) ) 
			{
				header('Content-type: application/text-stream');
				header('Content-Disposition: attachment; filename="'.$roster->name.'.xml"');
				Print $roster->roster;
			}
            Else
            {
                ...
                header('Content-Disposition: attachment; filename="'.$roster->name.'.db"');
