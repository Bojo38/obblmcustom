...
{
	global $db_prefix;
    $result = mysql_query("SELECT * FROM ".$db_prefix."hof WHERE hof_id = $hof_id");
...
{
global $db_prefix;
    if (mysql_query("UPDATE ".$db_prefix."hof SET 
...
{
global $db_prefix;
    return (mysql_query("DELETE FROM ".$db_prefix."hof WHERE hof_id = $this->hof_id"));
}
...
global $db_prefix;
    ...
    $result = mysql_query("SELECT hof_id, pid FROM ".$db_prefix."hof ORDER BY date DESC" . (($n) ? " LIMIT $n" : ''));
    if ($result && mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
...
public static function getModuleUpgradeSQL()
{
    ...
    return array(
        '075-080' => array(
            'CREATE TABLE IF NOT EXISTS '.$db_prefix.'hof
            (
                    hof_id  MEDIUMINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT,
                    pid     MEDIUMINT UNSIGNED,
                    date    DATETIME,
            ...
            'INSERT INTO '.$db_prefix.'hof (pid, date, title, about) SELECT f_id, date, txt2, txt FROM '.$db_prefix.'texts WHERE type = 5 ORDER BY date ASC',
            'DELETE FROM '.$db_prefix.'texts WHERE type = 5',
        ),
    );
}
...
{
global $db_prefix;
    $query = "SELECT pid FROM ".$db_prefix."hof WHERE pid = $pid";
