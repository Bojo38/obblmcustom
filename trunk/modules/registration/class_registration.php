...
#                 mysql_real_escape_string($this->username), $this->password, mysql_real_escape_string($this->email), NOT_ACTIVATED, ACCESS_LEVEL );
    ...
    private static function forgotform() {
        ...
        <form method='POST' action='handler.php?type=registration&form=forgot'>
            <div class='adminBox'>
                ...
                <div class='boxBody'>
                    Nom d'utilisateur :<br> <input type='text' name='new_name' size='20' maxlength='50'><br><br>
                    *Les instructions pour remettre votre<br>mot de passe à zéro <br>vous sera envoyé.
                    <br><br>
                    <input type='submit' name='button' value='RAZ Mot de passe'>
                </div>
            </div>
        </form>
        ";
    ...
    private static function forgotsubmitForm($username) {
        ...
        if ( !$register->error )
        {
            Print "Les instructions pour remettre votre mot de passe à zéro ont été envoyées à l'adresse fournie.";
            unset($register);
        }
        else
        {
            Print "<br><b>Erreur: {$register->error}</b><br>";
            unset($register);
            unset($_POST['new_name']);
            Registration::main(array());
        }
    ...
    function sendResetemail() {
        ...
        $to      = $this->email;
        $subject = 'Instructions de remise à zéro du mot de passe';
        $message = "Votre code de reset est: ".$this->reset_code;
        $headers = 'From: '.$webmaster. "\r\n" .
                   'Reply-To: '.$webmaster. "\r\n" .
                   'X-Mailer: PHP/' . phpversion();
