...
class Star 
{
    ...
    {
        global $STATS_TRANS,$db_prefix;
        $fields = array('cp','td','intcpt','mvp','bh+ki+si', 'bh','si','ki');
        $query = "SELECT ".implode(',', array_map(create_function('$f', 'return "SUM($f) AS \'$f\'";'), $fields))." FROM ".$db_prefix."match_data WHERE f_player_id = $this->star_id AND ".$STATS_TRANS[$type].'='.$type_id;
    ...
    {
	global $db_prefix;
        $query = "SELECT skills FROM ".$db_prefix."game_data_stars WHERE star_id = $this->star_id";
        $result = mysql_query($query);
    ...
    public function getHireHistory($obj, $obj_id, $node, $node_id)
    {
        ...
        */
        ...
        $query = "SELECT DISTINCT f_match_id, f_team_id FROM ".$db_prefix."match_data, ".$db_prefix."matches WHERE 
            f_match_id = match_id 
    ...
    public function rmMatchEntry($match_id, $team_id = false)
    {
        ...
        */
        global $db_prefix;
        $query = "DELETE FROM ".$db_prefix."match_data WHERE f_player_id = $this->star_id AND f_match_id = $match_id" . (($team_id) ? " AND f_team_id = $team_id" : '');
    ...
    public static function getStars($obj, $obj_id, $node, $node_id)
    {
        ...
        */
        ...
        global $stars, $STATS_TRANS,$db_prefix;
        $starObjs = array();
...
$query = "SELECT DISTINCT f_player_id FROM ".$db_prefix."match_data, ".$db_prefix."matches WHERE 
    ...
    */
    ...
    public function __construct($match_id, $nr)
    {
        ...
        */
        ...
        $query = "SELECT * FROM ".$db_prefix."match_data WHERE f_match_id = $match_id AND f_player_id = ".ID_MERCS." AND inj = $nr";
        $result = mysql_query($query);
	...
	global $db_prefix;
        ...
        */
        ...
        $query = "DELETE FROM ".$db_prefix."match_data WHERE f_player_id = ".ID_MERCS." AND f_match_id = $match_id".(($team_id) ? " AND f_team_id = $team_id" : '');
        return mysql_query($query);
    }
	...
	global $db_prefix;
        ...
        */
        ...
        $query = "SELECT inj, f_match_id FROM ".$db_prefix."match_data, ".$db_prefix."matches WHERE f_match_id = match_id AND f_team_id = $team_id AND f_player_id = ".ID_MERCS.(($f_match_id) ? " AND f_match_id = $f_match_id" : '') . ' ORDER BY date_played DESC, inj ASC';
        $result = mysql_query($query);
