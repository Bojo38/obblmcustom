...
class Coach
{
	...
	const T_RING_LOCAL_SCHEDULER    = 3; // League match scheduler
    ...
    public static $RINGS = array(
        ...
        self::T_RING_GROUP_LOCAL => array(self::T_RING_LOCAL_ADMIN,self::T_RING_LOCAL_SCHEDULER, self::T_RING_LOCAL_REGULAR, self::T_RING_LOCAL_NONE),
    );
    ...
    public function getTeams() {
        ...
        $result = mysql_query("SELECT team_id FROM ".$db_prefix."teams WHERE owned_by_coach_id = $this->coach_id ORDER BY name ASC");
	...
	global $db_prefix;
        ...
        $tours = array();
        $result = mysql_query("SELECT tour_id FROM ".$db_prefix."tours,".$db_prefix."teams WHERE winner = team_id AND owned_by_coach_id = $this->coach_id");
        while ($row = mysql_fetch_assoc($result)) {
	...
	global $db_prefix;
        ...
        if ($this->isDeletable()) {
            ...
            $status &= mysql_query("DELETE FROM ".$db_prefix."coaches WHERE coach_id = ".$this->coach_id);
        }
        else {
            $status = false;
        }
    ...
    public function setRetired($bool) {
        return mysql_query("UPDATE ".$db_prefix."coaches SET retired = ".(($bool) ? 1 : 0)." WHERE coach_id = $this->coach_id");
    }
    ...
    public function setRing($rtype, $ring, $lid = false) {
        ...
        if ($rtype == self::T_RING_GROUP_GLOBAL && in_array($ring, self::$RINGS[self::T_RING_GROUP_GLOBAL])) {
            $this->ring = $ring;
            return mysql_query("UPDATE ".$db_prefix."coaches SET ring = $ring WHERE coach_id = $this->coach_id");
        }
        else if ($rtype == self::T_RING_GROUP_LOCAL && in_array($ring, self::$RINGS[self::T_RING_GROUP_LOCAL]) && $lid) {
            $status = mysql_query("DELETE FROM ".$db_prefix."memberships WHERE cid = $this->coach_id AND lid = $lid");
            if ($ring != self::T_RING_LOCAL_NONE) {
                $status &= mysql_query("INSERT INTO ".$db_prefix."memberships (cid,lid,ring) VALUES($this->coach_id, $lid, $ring)");
            }
            return $status;
        }
    ...
    public function getAdminMenu()
    {
        ...
        $result = mysql_query("SELECT COUNT(*) FROM ".$db_prefix."memberships WHERE cid = $this->coach_id AND ring = ".self::T_RING_LOCAL_ADMIN);
        list($cnt) = mysql_fetch_row($result);
		
		$result_sch = mysql_query("SELECT COUNT(*) FROM ".$db_prefix."memberships WHERE cid = $this->coach_id AND ring >= ".self::T_RING_LOCAL_SCHEDULER);
        list($cnt_sch) = mysql_fetch_row($result_sch);
		
        $my_admin_menu = array_merge(
            $this->ring == Coach::T_RING_GLOBAL_ADMIN ? $ring_sch_access +$ring_com_access+$ring_sys_access: array(),
			$cnt_sch > 0 ? $ring_sch_access : array(),
	...
	global $db_prefix;
        $query = "UPDATE ".$db_prefix."coaches SET passwd = '".md5($passwd)."' WHERE coach_id = $this->coach_id";
	...
	global $db_prefix;
        ...
        $query = "UPDATE ".$db_prefix."coaches SET name = '".mysql_real_escape_string($name)."' WHERE coach_id = $this->coach_id";
	...
	global $db_prefix;
        $query = "UPDATE ".$db_prefix."coaches SET mail = '".mysql_real_escape_string($mail)."' WHERE coach_id = $this->coach_id";
        return (mysql_query($query) && ($this->mail = $mail));
    }

    public function setPhone($phnr) {
	global $db_prefix;
        $query = "UPDATE ".$db_prefix."coaches SET phone = '".mysql_real_escape_string($phnr)."' WHERE coach_id = $this->coach_id";
        return (mysql_query($query) && ($this->phone = $phnr));
    }

    public function setRealName($rname) {
	global $db_prefix;
        $query = "UPDATE ".$db_prefix."coaches SET realname = '".mysql_real_escape_string($rname)."' WHERE coach_id = $this->coach_id";
    ...
    public function isInMatch($match_id) {
         ...
         **/
		global $db_prefix;
        $result = mysql_query("SELECT team1_id, team2_id FROM ".$db_prefix."matches WHERE match_id = $match_id");
    ...
    public function setActivationCode($set_AC) 
    {
        ...
        if ($set_AC) {
            ...
            return mysql_query("UPDATE ".$db_prefix."coaches SET activation_code = '$AC' WHERE coach_id = $this->coach_id") ? $AC : false;
        }
        else {
            return mysql_query("UPDATE ".$db_prefix."coaches SET activation_code = NULL WHERE coach_id = $this->coach_id");
        }
    }
    ...
    {
	global $db_prefix;
        $query = "SELECT activation_code = '$AC' FROM ".$db_prefix."coaches WHERE coach_id = $this->coach_id";
    ...
    {
	global $db_prefix;
        $result = mysql_query("SELECT COUNT(*) FROM ".$db_prefix."coaches WHERE coach_id = $id");
        list($CNT) = mysql_fetch_row($result);
        return ($CNT == 1);
    }
    ...
    public static function allowedNodeAccess($NODE_SRUCT, $cid, $extraFields = array())
    {
        ...
        $query = "SELECT l.lid AS 'lid', d.did AS 'did', t.tour_id AS 'trid',".implode(',',$properFields)."
            FROM ".$db_prefix."leagues AS l LEFT JOIN ".$db_prefix."divisions AS d ON d.f_lid = l.lid LEFT JOIN ".$db_prefix."tours AS t ON t.f_did = d.did ".
            ((!$GLOBAL_VIEW) ? ", ".$db_prefix."memberships AS m WHERE m.lid = l.lid AND m.cid = $cid" : '');
        $result = mysql_query($query);
    ...
    public static function getCoaches() {
        ...
        $coaches = array();
        global $db_prefix;
        $query  = "SELECT coach_id FROM ".$db_prefix."coaches";
        $result = mysql_query($query);
        if ($result && mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
    ...
    public static function create(array $input) {
        ...
        $query = "INSERT INTO ".$db_prefix."coaches (name, realname, passwd, mail, phone, ring, settings) 
