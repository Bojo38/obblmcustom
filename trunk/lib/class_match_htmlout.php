...
{
    global $lng, $coach;
    global $leagues, $divisions, $tours,$db_prefix;
    ...
    $query = "SELECT COUNT(*) FROM ".$db_prefix."matches WHERE f_tour_id = $trid";
    ...
    $query = "SELECT t1.name AS 't1_name', t1.team_id AS 't1_id', t2.name AS 't2_name', t2.team_id AS 't2_id', match_id, date_played, locked, round, team1_score, team2_score 
        FROM ".$db_prefix."matches, ".$db_prefix."teams AS t1, ".$db_prefix."teams AS t2 WHERE f_tour_id = $trid AND team1_id = t1.team_id AND team2_id = t2.team_id 
...
public static function report_ES($mid, $DIS) 
{
    ...
    if (!$DIS && isset($_POST['ES_submitted'])) {
        $query = "SELECT tour_id AS 'trid', did, f_lid AS 'lid' FROM ".$db_prefix."matches, ".$db_prefix."tours, ".$db_prefix."divisions WHERE match_id = $mid AND f_tour_id = tour_id AND f_did = did";
        $result = mysql_query($query);
...
{
    global $ES_fields,$db_prefix;
    $query = "SELECT 
            ".$db_prefix."players.player_id AS 'pid', ".$db_prefix."players.owned_by_team_id AS 'f_tid', ".$db_prefix."players.f_cid AS 'f_cid', ".$db_prefix."players.f_rid AS 'f_rid',
            ".$db_prefix."players.name AS 'name', ".$db_prefix."players.nr AS 'nr',
            ".implode(',', array_keys($ES_fields))." 
        FROM ".$db_prefix."matches, ".$db_prefix."match_data, ".$db_prefix."players LEFT JOIN ".$db_prefix."match_data_es ON (".$db_prefix."match_data_es.f_mid = $mid AND ".$db_prefix."players.player_id = ".$db_prefix."match_data_es.f_pid)
        WHERE 
            ".$db_prefix."matches.match_id = $mid AND ".$db_prefix."matches.match_id = ".$db_prefix."match_data.f_match_id AND ".$db_prefix."match_data.f_player_id = ".$db_prefix."players.player_id AND (owned_by_team_id = team1_id OR owned_by_team_id = team2_id)
