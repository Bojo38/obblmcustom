...
if (isset($_POST['button'])) {
    ...
    */
    ...
    foreach ($team_ids as $tid) {
        $query = "SELECT (t.f_did = $did) AS 'in_did', (t.f_lid = $lid) AS 'in_lid' FROM ".$db_prefix."teams AS t WHERE t.team_id = $tid";
    ...
    $errors = array(
        ...
        array($leagues[$lid]['ring'] < Coach::T_RING_LOCAL_SCHEDULER, 'You do not have the rights to schedule matches in the selected league.'),
...
<form method="POST" name="tourForm">
    ...
    <td valign='top'>
        ...
        <select name='did'>
            ...
            foreach ($divisions as $did => $desc) {
                if ($leagues[$desc['f_lid']]['ring'] >= Coach::T_RING_LOCAL_SCHEDULER) {
    ...
    <td valign='top'>
        ...
        foreach ($tours as $trid => $desc) {
            if ($desc['type'] == TT_FFA && $leagues[$divisions[$desc['f_did']]['f_lid']]['ring'] >= Coach::T_RING_LOCAL_SCHEDULER) {
