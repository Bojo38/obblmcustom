...
switch ($_GET['type'])
{
    ...
    case 'autocomplete':
        ...
        switch ($_GET['obj']) {
            ...
            case T_OBJ_COACH:
                $query = "SELECT coach_id AS 'id', name FROM ".$db_prefix."coaches WHERE name LIKE '$_GET[query]%' ORDER BY name ASC";
            ...
            case T_OBJ_TEAM:
                $query = "SELECT team_id AS 'id', name FROM ".$db_prefix."teams WHERE name LIKE '%$_GET[query]%' ORDER BY name ASC";
                $result = mysql_query($query);
                while($row = mysql_fetch_assoc($result)) {
                    $objs[$row['id']] = $row['name'];
                }
