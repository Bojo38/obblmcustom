...
{
    global $core_tables, $relations_obj, $relations_node, $objFields_extra, $objFields_init,$db_prefix;
    ...
    $val0 = ($ALL_TIME && !empty($objFields_val0)) ? '(in_array($c,array_keys($objFields_val0)) ? "'.$db_prefix.$tbl_name_norm.'.".$objFields_val0[$c]."+" : "")' : '""';
    ###
    $cols_norm_sqlsel = implode(',', array_map(
        create_function('$c', $f1='
            global $cols_mv, $objFields_val0; 
            return '.$val0.'."'.$db_prefix.$tbl_name_norm.'.$c AS \'".((in_array($c,$cols_mv))?"rg_":"")."$c\'\\n";'
        ), 
        $cols_norm)
    );
    $cols_mv_sqlsel = implode(',', array_map(
        create_function('$c', $f2='
            ...
            return '.$val0.'."IFNULL(SUM('.$db_prefix.$tbl_name_mv.'.$c)'.(($setAvg) ? '".(in_array($c,$objFields_avg) ? "/SUM('.$db_prefix.$tbl_name_mv.'.played)" : "")."' : "").',0) AS \'mv_$c\'\\n";'
        ), 
        $cols_mv)
    );
    $cols_mv_es_sqlsel = implode(',', array_map( # Does NOT have init or extra value ($val0) additions!
        create_function('$c', $f3='
            return "IFNULL(SUM('.$db_prefix.$tbl_name_mv_es.'.$c)'.(($setAvg) ? '/SUM('.$db_prefix.$tbl_name_mv.'.played)' : "").',0) AS \'mv_$c\'\\n";'
        ), 
        $cols_mv_es)
    );
    ...
    $MV    = $db_prefix.$tbl_name_mv.'.'.$mv_keys[$obj];
    $MV_ES = $db_prefix.$tbl_name_mv_es.'.'.$mv_keys[$obj];
    $REG   = $db_prefix.$tbl_name_norm.'.'.$relations_obj[$obj]['id'];
    $query = "SELECT $cols_mv_sqlsel,$cols_norm_sqlsel,$cols_mv_es_sqlsel
        FROM ".$db_prefix."$tbl_name_norm LEFT JOIN (".$db_prefix."$tbl_name_mv, ".$db_prefix."$tbl_name_mv_es) ON ($MV = $MV_ES AND ".$db_prefix."$tbl_name_mv.f_trid = ".$db_prefix."$tbl_name_mv_es.f_trid AND $MV = $REG)";
    ...
    foreach ($filters as $filter_key => $id) {
        if (is_numeric($id) && is_numeric($filter_key)) {
            ...
            if ($obj == $filter_key) {
                $where .= $db_prefix.$tbl_name_norm.'.'.$relations_obj[$filter_key]['id']." = $id ";
            }
            elseif ($filter_key == $obj+1 && ($obj == T_OBJ_PLAYER || $obj = T_OBJ_TEAM)) {
                $where .= $db_prefix.$tbl_name_norm.'.'.$relations_obj[$obj]['parent_id']." = $id ";
            }
            else {
                $where .= $db_prefix.$tbl_name_mv.'.'.$mv_keys[$filter_key]." = $id ";
            }
        }
    }
...
private static function buildQueryComponents($obj, $obj_id, $node, $node_id, $TTS = '') # TTS = Teams Table Suffix
{
    ...
    */
    ...
    $from = array($db_prefix.'matches',$db_prefix.'tours',$db_prefix.'divisions'); // We don't need to add the "leagues" table, since league IDs can be referenced via the "f_lid" key in the divisions table.
    $where = array($db_prefix."matches.f_tour_id = ".$db_prefix."tours.tour_id", $db_prefix."tours.f_did = ".$db_prefix."divisions.did");
    if ($node && $node_id) {
        switch ($node)
        {
            case STATS_TOUR:        $where[] = $db_prefix."matches.f_tour_id   = $node_id"; break;
            case STATS_DIVISION:    $where[] = $db_prefix."tours.f_did         = $node_id"; break;
            case STATS_LEAGUE:      $where[] = $db_prefix."divisions.f_lid     = $node_id"; break;
        }
    }
    ...
    {
        case STATS_PLAYER:  
            array_push($from, $db_prefix."teams AS teams$TTS", $db_prefix."players AS players$TTS"); # Also use $TTS for players table: If not included calling buildCrossRefQueryComponents() will accidentally create two tables with identical names (if comparing two players).
            ...
            $from[]    = "(SELECT DISTINCT(f_match_id) AS 'p_mid' FROM ".$db_prefix."match_data WHERE f_player_id = $obj_id AND mg IS FALSE) AS pmatches$TTS"; # Also use $TTL here, same reason as above for players table.
...
//            array_push($where, "");
        ...
        case STATS_RACE:
            array_push($from, $db_prefix."teams AS teams$TTS");
            $tid = "teams$TTS.team_id";
