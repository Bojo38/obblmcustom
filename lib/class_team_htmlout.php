...
public static function dispList()
{
    ...
    */
    ...
    $fields = '_RRP AS "team_id", owned_by_coach_id, f_race_id, '.$db_prefix.'teams.name AS "tname", f_cname, f_rname, tv, '.$db_prefix.'teams.rdy AS "rdy", '.$db_prefix.'teams.retired AS "retired"';
    $where = array();
    if ($sel_state == T_STATE_ACTIVE) $where[] = $db_prefix.'teams.rdy IS TRUE AND '.$db_prefix.'teams.retired IS FALSE';
    if ($sel_race != T_RACE_ALL) 	  $where[] = $db_prefix."teams.f_race_id = $sel_race";
    ...
    if ($sel_node == T_NODE_LEAGUE || $ALL_TIME) {
        ...
        $queryCnt = "SELECT COUNT(*) FROM ".$db_prefix."teams $where";
...
private function _roster($ALLOW_EDIT, $DETAILED, $players)
{
    ...
    foreach ($players as $p) {
        ...
        */      
        ...
        if ($ALLOW_EDIT && $p->mayHaveNewSkill()) {
            ...
            $x .= "<optgroup label='Comp�tences Normales'>\n";
            foreach ($p->choosable_skills['norm'] as $s) {
                $x .= "<option value='$s'>".$skillididx[$s]."</option>\n";
            }
            $x .= "</optgroup>\n";

            $x .= "<optgroup label='Comp�tences Doubles'>\n";
            foreach ($p->choosable_skills['doub'] as $s) {
                $x .= "<option value='$s'>".$skillididx[$s]."</option>\n";
            }
            $x .= "</optgroup>\n";
            
            $x .= "<optgroup label='Augmentations de caract�ristiques'>\n";
    ...
    $fields = array(
        ...
        'mv_cas'    => array('desc' => ($DETAILED) ? 'Leg/Gra/Tu�' : 'Sor', 'nosort' => ($DETAILED) ? true : false),
...
private function _menu($ALLOW_EDIT, $DETAILED)
{
    ...
    <ul id="nav" class="dropdown dropdown-horizontal" style="position:static; z-index:0;">
        ...
        <?php
        echo "<li><a href='${url}&amp;subsec=hhstar' title='Show/hide star hire history'>Hist. Champ.</a></li>\n";
        echo "<li><a href='${url}&amp;subsec=hhmerc' title='Show/hide mercenary hire history'>Hist. Merc.</a></li>\n";
        ...
        if ($pdf || $botocs) {
        ?>
        <li><span class="dir">Exporter Roster</span>
            <ul>
        ...
        if (Module::isRegistered('IndcPage')) {
            echo "<li><a href='handler.php?type=inducements&amp;team_id=$team->team_id'>Sim. Primes de match</a></li>\n";
        }
        if (Module::isRegistered('SGraph')) {
            echo "<li><a href='handler.php?type=graph&amp;gtype=".SG_T_TEAM."&amp;id=$team->team_id''>Stats</a></li>\n";
        }
        ?>
    </ul>
    <br><br>
