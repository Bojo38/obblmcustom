...
{
	global $db_prefix;
    $result = mysql_query("SELECT * FROM ".$db_prefix."divisions WHERE did = $did");
...
{
	global $db_prefix;
    return mysql_query("DELETE FROM ".$db_prefix."divisions WHERE did = $this->did");
}

public function setName($name)
{
	$db_prefix;
    $query = "UPDATE ".$db_prefix."divisions SET name = '".mysql_real_escape_string($name)."' WHERE did = $this->did";
...
public function getTours($onlyIds = false)
{
	global $db_prefix;
    $tours = array();
    $result = mysql_query("SELECT tour_id FROM ".$db_prefix."tours WHERE f_did = $this->did");
...
public static function getDivisions($onlyIds = false)
{
	global $db_prefix;
    $divisions = array();
    $result = mysql_query("SELECT did FROM ".$db_prefix."divisions");
    if ($result && mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
...
{
	global $db_prefix;
    $query = "INSERT INTO ".$db_prefix."divisions (f_lid, name) VALUES ($f_lid, '".mysql_real_escape_string($name)."')";
    return (get_alt_col('divisions', 'name', $name, 'did')) ? false : mysql_query($query);
}
