...
class Team
{
	...
	global $db_prefix;
        $query = "UPDATE ".$db_prefix."teams SET owned_by_coach_id = $cid WHERE team_id = $this->team_id";
	...
	global $db_prefix;
        $this->_players = array();
        $result = mysql_query("SELECT player_id FROM ".$db_prefix."players WHERE owned_by_team_id = $this->team_id ORDER BY nr ASC, name ASC");
...
global $db_prefix;
        ...
        $query = "SELECT f_tour_id FROM ".$db_prefix."matches WHERE team1_id = $this->team_id OR team2_id = $this->team_id ORDER BY date_played DESC LIMIT 1";
...
global $db_prefix;
        ...
        if ($this->isDeletable()) {
            $query = "DELETE FROM ".$db_prefix."match_data WHERE f_team_id = $this->team_id"; mysql_query($query); // These entries occur only when players are imported.
            $query = "DELETE FROM ".$db_prefix."players WHERE owned_by_team_id = $this->team_id"; mysql_query($query);
            $query = "DELETE FROM ".$db_prefix."teams WHERE team_id = $this->team_id"; mysql_query($query);
            $query = "DELETE FROM ".$db_prefix."mv_players WHERE f_tid = $this->team_id"; mysql_query($query);
            $query = "DELETE FROM ".$db_prefix."mv_teams WHERE f_tid = $this->team_id"; mysql_query($query);
...
global $db_prefix;
        ...
        $query  = "SELECT team_id FROM ".$db_prefix."teams WHERE team_id != $this->team_id AND name = '" . mysql_real_escape_string($new_name) . "'";
        $result = mysql_query($query);
        if (mysql_num_rows($result) > 0)
            return false;

        $query = "UPDATE t".$db_prefix."eams SET name = '" . mysql_real_escape_string($new_name) . "' WHERE team_id = $this->team_id";
...
global $db_prefix;
        return mysql_query("UPDATE ".$db_prefix."teams SET retired = ".(($bool) ? 1 : 0)." WHERE team_id = $this->team_id");
    }
...
global $db_prefix;
        ...
        if (mysql_query("UPDATE ".$db_prefix."teams SET treasury = treasury - $price, $thing = $thing + 1 WHERE team_id = $this->team_id")) {
...
global $db_prefix;
        ...
        $query = "UPDATE ".$db_prefix."teams SET treasury = treasury + $delta WHERE team_id = $this->team_id";
...
global $db_prefix;
        mysql_query("UPDATE ".$db_prefix."teams SET rdy = ".(($bool) ? 1 : 0)." WHERE team_id = $this->team_id");
        $t->rdy = $bool;
        return true;
    }
...
global $db_prefix;
        ...
        $query = "SELECT match_id FROM ".$db_prefix."matches WHERE team1_id = $this->team_id OR team2_id = $this->team_id LIMIT 1";
        $result = mysql_query($query);
...
global $db_prefix;
        ...
        $query = "SELECT (COUNT(*) >= ".$rules['max_team_players'].") FROM ".$db_prefix."players
    ...
    public function isPlayerBuyable($pos_id) {
        ...
        */
        ...
        $query = "SELECT IFNULL(COUNT(*) < qty, TRUE) FROM ".$db_prefix."players, ".$db_prefix."game_data_players 
	...
	global $db_prefix;
        return SQLBoolEval("SELECT COUNT(*) FROM ".$db_prefix."players WHERE owned_by_team_id = $this->team_id AND nr = $nr AND date_sold IS NULL AND date_died IS NULL AND status != ".DEAD);
    }
    
    public function isJMLimitReached() {
        global $rules;
        return ($rules['journeymen_limit'] <= (int) SQLFetchField("SELECT COUNT(*) FROM ".$db_prefix."players WHERE owned_by_team_id = $this->team_id AND date_sold IS NULL AND date_died IS NULL AND status = ".NONE));
    }
    ...
    public function getToursPlayedIn($ids_only = false)
    {
        ...
        $query = "SELECT DISTINCT(f_tour_id) FROM ".$db_prefix."matches, ".$db_prefix."tours
                WHERE f_tour_id = tour_id AND team1_id = $this->team_id OR team2_id = $this->team_id
                ORDER BY ".$db_prefix."tours.date_created ASC";
        $result = mysql_query($query);
        if (mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
    ...
    public function getFreePlayerNr()
    {
		...
		global $db_prefix;
        $query = "SELECT GROUP_CONCAT(nr) FROM ".$db_prefix."players WHERE owned_by_team_id = $this->team_id GROUP BY owned_by_team_id";
        $result = mysql_query($query);
    ...
    {
	global $db_prefix;
        $result = mysql_query("SELECT COUNT(*) FROM ".$db_prefix."teams WHERE team_id = $id");
    ...
    public static function getTeams($race_id = false) {
        ...
        $query = "SELECT team_id FROM ".$db_prefix."teams" . (($race_id !== false) ? " WHERE f_race_id=$race_id" : '');
        $result = mysql_query($query);
        if (mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
    ...
    public static function create(array $input) {
        ...
        $errors = array(
            ...
            self::T_CREATE_ERROR__INVALID_LEAGUE   => get_alt_col('coaches', 'coach_id', (int) $input['owned_by_coach_id'], 'ring') != Coach::T_RING_GLOBAL_ADMIN && 0 == (int) SQLFetchField("SELECT COUNT(*) FROM ".$db_prefix."memberships WHERE lid = ".((int) $input['f_lid'])." AND cid = ".((int) $input['owned_by_coach_id'])." AND ring >= ".Coach::T_RING_LOCAL_REGULAR),
        ...
        $query = "INSERT INTO ".$db_prefix."teams (".implode(',',$EXPECTED).") VALUES (".implode(',', $input).")";
        if (mysql_query($query))
            $tid = mysql_insert_id();
        else {
