...
class TextSubSys
{
    ...
    {
		global $db_prefix;
        $result = mysql_query("SELECT * FROM ".$db_prefix."texts WHERE txt_id = $txt_id");
    ...
    {
	global $db_prefix;
        return (mysql_query("DELETE FROM ".$db_prefix."texts WHERE txt_id = $this->txt_id"));
    }
    
    public function edit($txt, $txt2, $f_id = false, $type = false)
    {
	global $db_prefix;
        if (mysql_query("UPDATE ".$db_prefix."texts SET 
    ...
    {
	global $db_prefix;
        $query = "INSERT INTO ".$db_prefix."texts (f_id, txt2, txt, date, type".(($f_id2) ? ', f_id2' : '').") 
...
class ObjDescriptions extends TextSubSys
{
    ...
    {
	global $db_prefix;
        $result = mysql_query("SELECT * FROM ".$db_prefix."texts WHERE type = $type AND f_id = $f_id");
        if ($result && mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
                foreach ($row as $key => $val) {
                    $this->$key = $val;
...
class MatchSummary extends TextSubSys
{
    ...
    */
	...
	global $db_prefix;
        $this->match_id = $mid;
        $query = "SELECT txt_id FROM ".$db_prefix."texts WHERE f_id = $mid AND type = ".T_TEXT_MATCH_SUMMARY;
    ...
    public static function getSummaries($n = false, $lid = false) {
        ...
        $query = "SELECT match_id, txt FROM ".$db_prefix."matches, ".$db_prefix."tours, ".$db_prefix."divisions, ".$db_prefix."texts WHERE 
                f_tour_id = tour_id
            AND f_did = did
            AND match_id = f_id 
            AND ".$db_prefix."texts.type = ".T_TEXT_MATCH_SUMMARY." 
...
class TeamNews extends TextSubSys
{
    ...
    */
    ...
    public static function getNews($tid = false, $n = false, $lid = false)
    {
        ...
        $query = "SELECT txt_id FROM ".$db_prefix."texts, ".$db_prefix."teams 
