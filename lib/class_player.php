...
class Player
{
    ...
    function __construct($player_id) {
        ...
        global $DEA,$db_prefix;

        // Get relaveant store game data.
        $result = mysql_query("SELECT player_id,
            ".$db_prefix."game_data_players.qty AS 'qty', ".$db_prefix."game_data_players.pos AS 'pos', ".$db_prefix."game_data_players.skills AS 'def_skills', ".$db_prefix."game_data_players.botocs_name AS 'pos_botocs',
            ".$db_prefix."game_data_players.ma AS 'def_ma', ".$db_prefix."game_data_players.st AS 'def_st', ".$db_prefix."game_data_players.ag AS 'def_ag', ".$db_prefix."game_data_players.av AS 'def_av'
            FROM ".$db_prefix."players, ".$db_prefix."game_data_players WHERE player_id = $player_id AND f_pos_id = pos_id");
    ...
    public function setSkills() {
        global $skillcats,$db_prefix;
        foreach ($skillcats as $t => $grp) {
            $result = mysql_query("SELECT GROUP_CONCAT(f_skill_id) FROM ".$db_prefix."players_skills WHERE f_pid = $this->player_id AND type = '$t'");
    ...
    public function setChoosableSkills() {
        ...
        global $DEA, $skillarray, $skillcats,$db_prefix;
        ...
        FROM ".$db_prefix."match_data, ".$db_prefix."matches WHERE f_match_id = match_id AND f_player_id = $this->player_id AND (ir1_d1 != 0 OR ir1_d2 != 0 OR ir2_d1 != 0 OR ir2_d2 != 0 OR ir3_d1 != 0 OR ir3_d2 != 0) ORDER BY date_played DESC LIMIT $N_allowed_new_skills";
    ...
    public function is_unbuyable() {
        ...
        $query = "SELECT COUNT(*) AS 'cnt' FROM ".$db_prefix."match_data WHERE f_player_id = $this->player_id";
    ...
    public function sell() {
        ...
        if (!mysql_query("UPDATE ".$db_prefix."players SET date_sold = NOW() WHERE player_id = $this->player_id"))
            return false;
    ...
    public function unsell() {
        ...
        if (!mysql_query("UPDATE ".$db_prefix."players SET date_sold = NULL WHERE player_id = $this->player_id"))
            return false;
    ...
    public function unbuy() { # "Un-create"
        ...
        if (!mysql_query("DELETE FROM ".$db_prefix."players WHERE player_id = $this->player_id"))
            return false;
    ...
    public function hireJourneyman() {
        ...
        $query = "UPDATE ".$db_prefix."players SET type = ".PLAYER_TYPE_NORMAL." WHERE player_id = $this->player_id";
    ...
    public function unhireJourneyman() {
        ...
        $query = "UPDATE ".$db_prefix."players SET type = ".PLAYER_TYPE_JOURNEY." WHERE player_id = $this->player_id";
    ...
    public function rename($new_name) {
		global $db_prefix;
        return mysql_query("UPDATE ".$db_prefix."players SET name = '" . mysql_real_escape_string($new_name) . "' WHERE player_id = $this->player_id");
    }
    
    public function renumber($number) {
        global $T_ALLOWED_PLAYER_NR,$db_prefix;
        return (in_array($number, $T_ALLOWED_PLAYER_NR) && mysql_query("UPDATE ".$db_prefix."players SET nr = $number WHERE player_id = $this->player_id"));
    }

    public function dspp($delta) {
		global $db_prefix;
        $query = "UPDATE ".$db_prefix."players SET extra_spp = IF(extra_spp IS NULL, $delta, extra_spp + ($delta)) WHERE player_id = $this->player_id";
        return mysql_query($query);
    }

    public function dval($val = 0) {
		global $db_prefix;
        $query = "UPDATE ".$db_prefix."players SET extra_val = $val WHERE player_id = $this->player_id";
    ...
    public function addSkill($type, $skill) {
        ...
        global $DEA, $skillididx, $skillcats, $CHR_CONV,$db_prefix;
        ...
        if ($type == "C" && in_array($skill, $this->choosable_skills['chr'])) {
            ...
            $query = "UPDATE ".$db_prefix."players SET ach_$fname = ach_$fname + 1 WHERE player_id = $this->player_id";
        }
        elseif ($IS_REGULAR || $IS_EXTRA) {
            $this->{$skillcats[$type]['obj_idx']}[] = $skill;
            $query = "INSERT INTO ".$db_prefix."players_skills(f_pid, f_skill_id, type) VALUES ($this->player_id, $skill, '$type')";
        }
    ...
    public function rmSkill($type, $skill) {
        ...
        global $skillcats, $CHR_CONV,$db_prefix;

        $query = '';
        if (in_array($type, array_keys($skillcats))) {
            $query = "DELETE FROM ".$db_prefix."players_skills WHERE f_pid = $this->player_id AND type = '$type' AND f_skill_id = $skill";
        }
        elseif ($type == 'C') {
            $fname = $CHR_CONV[$skill];
            $query = "UPDATE ".$db_prefix."players SET ach_$fname = ach_$fname - 1 WHERE player_id = $this->player_id";
    ...
    public function getMatchMost($field) {
        ...
        $matchesPlayed = "(SELECT DISTINCT f_match_id AS 'mid' FROM ".$db_prefix."match_data WHERE f_player_id = $this->player_id) AS matchesPlayed";
        $max = "(SELECT f_match_id AS 'mid', MAX($field) AS 'maxVal' FROM ".$db_prefix."match_data, $matchesPlayed WHERE f_match_id = mid GROUP BY f_match_id) AS max";
        $cntMax = "(SELECT f_match_id AS 'mid', COUNT(*) AS 'cnt', maxVal FROM ".$db_prefix."match_data, $max WHERE f_match_id = mid AND ($field) = maxVal GROUP BY f_match_id) cntMax";
        $query = "
            SELECT 
                *
            FROM 
                ".$db_prefix."match_data, $cntMax 
            WHERE 
                    f_match_id = mid 
    ...
    public function getAchEntries($type) {
        ...
        $query = "SELECT mvp, cp, td, intcpt, bh, ki, si, f_match_id FROM ".$db_prefix."match_data, ".$db_prefix."matches WHERE match_id > 0 AND f_match_id = match_id AND f_player_id = $this->player_id AND ($type) > 0 ORDER BY date_played DESC";
        if (($result = mysql_query($query)) && mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
	...
	public function getSkillsStrBotocs($HTML = false) 
    {
        /**
         * Compiles skills string.
         **/
		...
		$skills=array();
		for ($i=0;$i<count($this->extra_skills);$i++)
		{
			array_push($skills,get_alt_col('game_data_skills','skill_id',$this->extra_skills[$i],'botocs_name'));
		}
		for ($i=0;$i<count($this->def_skills);$i++)
		{
			array_push($skills,get_alt_col('game_data_skills','skill_id',$this->def_skills[$i],'botocs_name'));
		}
		for ($i=0;$i<count($this->ach_nor_skills);$i++)
		{
			array_push($skills,get_alt_col('game_data_skills','skill_id',$this->ach_nor_skills[$i],'botocs_name'));
		}
		for ($i=0;$i<count($this->ach_dob_skills);$i++)
		{
			array_push($skills,get_alt_col('game_data_skills','skill_id',$this->ach_dob_skills[$i],'botocs_name'));
		}
		...
		if ($this->ach_ma > 0) array_push($skills, "+$this->ach_ma Ma");
        if ($this->ach_st > 0) array_push($skills, "+$this->ach_st St");
        if ($this->ach_ag > 0) array_push($skills, "+$this->ach_ag Ag");
        if ($this->ach_av > 0) array_push($skills, "+$this->ach_av Av");
    ...
    {
		global $db_prefix;
        $result = mysql_query("SELECT COUNT(*) FROM ".$db_prefix."players WHERE player_id = $id");
    ...
    public static function price($pos_id) {
         ...
         **/
        global $db_prefix;
        $result = mysql_query("SELECT cost FROM ".$db_prefix."game_data_players WHERE pos_id = $pos_id");
        $row = mysql_fetch_row($result);
        return (int) $row[0];
    }
    ...
    public static function create(array $input, array $opts) {
        ...
        global $rules, $DEA, $T_ALL_PLAYER_NR,$db_prefix;
        ...
        $query = "INSERT INTO ".$db_prefix."players (".implode(',',array_keys($input)).") VALUES (".implode(',', array_values($input)).")";
        if (mysql_query($query)) {
            $pid = mysql_insert_id();
            $team->dtreasury(-1 * $price);
        }
