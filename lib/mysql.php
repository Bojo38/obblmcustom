...
"pass_attempts" => array("short" => "cp_a","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de tentatives de passes"),
"interceptions_thrown" => array("short" => "cp_int","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de fois où le lanceur a été intercepté."),
"safe_throws" => array("short" => "cp_st","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre d'interceptions annulées par Lancer précis."),
"pass_distance" => array("short" => "cp_dist","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de cases gagnées par passe."),
"dumpoff_attempts" => array("short" => "dmp_a","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de passes lancées qui on été transmises (La nombre passes inclu les transmissions)."),
"dumpoff_completions" => array("short" => "dmp","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de transmission réussies (Les réussites de transmission sont inclues dans les passes réussies)."),
"catch_attempts" => array("short" => "catch_a","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de tentatives de réception réalisées depuis un lancer."),
"catches" => array("short" => "catch","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de réception réalisées (relances incluses)."),
"handoffs" => array("short" => "hnd","type" => "MEDIUMINT SIGNED NOT NULL DEFAULT 0", "group" => "Offensive stats", "desc" => "Nombre de transmissions rélisées"),
...
function mysql_up($do_table_check = false) {
    ...
    if ($do_table_check) {
        ...
        $tables_expected = array_keys($core_tables);
			...
			$tables_expected2=array();
			foreach($tables_expected as $exp)
			{
				array_push ($tables_expected2,$db_prefix.$exp);
			}
        ...
        $tables_diff = array_diff($tables_expected2, $tables_found);
...
function get_alt_col($V, $X, $Y, $Z) {
     ...
     */
	 ...
	 global $db_prefix;
    $result = mysql_query("SELECT $Z FROM ".$db_prefix."$V WHERE $X = '" . mysql_real_escape_string($Y) . "'");
...
function get_rows($tbl, array $getFields, $where = array()) {
    ...
    */
    $query = 'SELECT '.(empty($getFields) ? '*' : implode(',', $getFields))." FROM ".$db_prefix."$tbl ".(empty($where) ? '' : 'WHERE '.implode(' AND ', $where));
    $ret = array();
...
function get_parent_id($type, $id, $parent_type) {
    global $relations_node, $relations_obj,$db_prefix;
	...
	{
		$tables[$i]=$db_prefix.$tables[$i];
	}
	
    array_pop($REL_trimmed);
    array_shift($REL_trimmed_padded);
    $wheres = array_map(create_function('$rl,$rl_next', 'return "'.$db_prefix.'$rl[tbl].$rl[parent_id] = '.$db_prefix.'$rl_next[tbl].$rl_next[id]";'), $REL_trimmed, $REL_trimmed_padded);
...
function get_list($table, $col, $val, $new_col) {
	global $db_prefix;
    $result = mysql_query("SELECT $new_col FROM ".$db_prefix."$table WHERE $col = '$val'");
...
function set_list($table, $col, $val, $new_col, $new_val = array()) {
    ...
    if (mysql_query("UPDATE ".$db_prefix."$table SET $new_col = '$new_val' WHERE $col = '$val'")) 
        return true;
    else
        return false;
}
...
class SQLUpgrade
{
    ...
    public static function doesColExist($tbl, $col)
    {
        ...
        $colCheck = "SELECT EXISTS(SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$db_name' AND COLUMN_NAME='$col' AND TABLE_NAME='".$db_prefix."$tbl') AS 'exists'";
