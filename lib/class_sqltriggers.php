    ...
    protected static function _player($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            ...
            case T_SQLTRIG_PLAYER_DPROPS: 
                ...
                mysql_query("UPDATE ".$db_prefix."players SET 
            ...
            case T_SQLTRIG_PLAYER_RELS: 
                ...
                mysql_query("UPDATE ".$db_prefix."players SET f_cid = @f_cid, f_rid = @f_rid, f_cname = @f_cname, f_rname = @f_rname, f_tname = @f_tname, f_pos_name = @f_pos_name WHERE player_id = $pid") or die(mysql_error());
                break;
        }
    }
    ...
    protected static function _team($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            ...
            case T_SQLTRIG_TEAM_DPROPS: 
                ...
                mysql_query("UPDATE ".$db_prefix."teams SET tv = @tv, ff = @ff WHERE team_id = $tid") or die(mysql_error());
                break;
            
            case T_SQLTRIG_TEAM_RELS: 
                mysql_query("CALL getTeamRels($tid, @f_cname, @f_rname)") or die(mysql_error());
                mysql_query("UPDATE ".$db_prefix."teams SET f_cname = @f_cname, f_rname = @f_rname WHERE team_id = $tid") or die(mysql_error());
                break;
                
            case T_SQLTRIG_TEAM_UPDATE_CHILD_RELS: 
                mysql_query("UPDATE ".$db_prefix."players,".$db_prefix."teams SET f_tname = teams.name, f_cid = owned_by_coach_id, players.f_cname = teams.f_cname WHERE team_id = $tid AND owned_by_team_id = team_id") or die(mysql_error());
                mysql_query("UPDATE ".$db_prefix."mv_players,".$db_prefix."teams SET f_cid = owned_by_coach_id WHERE team_id = $tid AND f_tid = team_id") or die(mysql_error());
                mysql_query("UPDATE ".$db_prefix."mv_teams,".$db_prefix."teams SET f_cid = owned_by_coach_id WHERE team_id = $tid AND f_tid = team_id") or die(mysql_error());
                self::_coach(T_SQLTRIG_COACH_TEAMCNT, array('id' => $t->owned_by_coach_id, 'obj' => new Coach($t->owned_by_coach_id)));
                break;
        }
    }
    ...
    protected static function _coach($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            case T_SQLTRIG_COACH_UPDATE_CHILD_RELS: 
                mysql_query("UPDATE ".$db_prefix."players,".$db_prefix."coaches SET f_cname = ".$db_prefix."coaches.name WHERE coach_id = $cid AND f_cid = coach_id") or die(mysql_error());
                mysql_query("UPDATE ".$db_prefix."teams,".$db_prefix."coaches SET f_cname = ".$db_prefix."coaches.name WHERE coach_id = $cid AND owned_by_coach_id = coach_id") or die(mysql_error());
                break;
                
            case T_SQLTRIG_COACH_TEAMCNT:
                mysql_query("UPDATE ".$db_prefix."coaches SET team_cnt = getTeamCnt(".T_OBJ_COACH.", $cid, NULL) WHERE coach_id = $cid") or die(mysql_error());
                break;
        }
    }
    ...
    protected static function _race($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            case T_SQLTRIG_RACE_TEAMCNT:
                mysql_query("UPDATE ".$db_prefix."races SET team_cnt = getTeamCnt(".T_OBJ_RACE.", $rid, NULL) WHERE race_id = $rid") or die(mysql_error());
                break;
        }
    }
