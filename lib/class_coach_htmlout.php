...
public static function dispList()
{
    ...
    */
    ...
    $fields = 'coach_id AS "coach_id", '.$db_prefix.'coaches.name AS "cname", '.$db_prefix.'coaches.retired AS "retired", team_cnt AS "team_cnt"';
    $where = array();
    if ($sel_state == T_STATE_ACTIVE) $where[] = $db_prefix.'coaches.retired IS FALSE';

    if ($ALL_TIME) {
        $where = (count($where) > 0) ? 'WHERE '.implode(' AND ', $where) : '';
	    $queryCnt = "SELECT COUNT(*) FROM ".$db_prefix."coaches $where";
	    $queryGet = "SELECT $fields FROM ".$db_prefix."coaches $where ORDER BY cname ASC";
	}
	    ...
	    $queryCnt = "SELECT COUNT(DISTINCT cid, lid) FROM ".$db_prefix."coaches,".$db_prefix."memberships WHERE cid = coach_id AND lid = $sel_node_id $where";
	    $queryGet = "SELECT $fields FROM ".$db_prefix."coaches,".$db_prefix."memberships WHERE cid = coach_id AND lid = $sel_node_id $where GROUP BY cid, lid ORDER BY cname ASC";
	}
    else {
        $q = "SELECT $fields FROM ".$db_prefix."matches, ".$db_prefix."teams, ".$db_prefix."coaches, ".$db_prefix."tours, ".$db_prefix."divisions 
              WHERE ".$db_prefix."teams.owned_by_coach_id = ".$db_prefix."coaches.coach_id AND ".$db_prefix."matches._RRP = ".$db_prefix."teams.team_id AND ".$db_prefix."matches.f_tour_id = ".$db_prefix."tours.tour_id AND ".$db_prefix."tours.f_did = ".$db_prefix."divisions.did ";
	    switch ($sel_node)
	    {
	        case false: break;
	        case T_NODE_TOURNAMENT: $q .= "AND ".$db_prefix."tours.tour_id = $sel_node_id";   break;
	        case T_NODE_DIVISION:   $q .= "AND ".$db_prefix."divisions.did = $sel_node_id";   break;
	        case T_NODE_LEAGUE:     $q .= "AND ".$db_prefix."divisions.f_lid = $sel_node_id"; break;
	    }
    ...
    <div class="row">
        <div class="boxCoachPage">
            ...
            <div class='boxBody'>
                ...
                <?php
                echo "<tr><td>".$lng->getTrn('common/Played')."</td><td>$this->mv_played</td></tr>\n";
                echo "<tr><td>".$lng->getTrn('common/Winpct')."</td><td>".(sprintf("%1.1f", $this->rg_win_pct).'%')."</td></tr>\n";
                echo "<tr><td>ELO</td><td>".(($this->rg_elo) ? sprintf("%1.2f", $this->rg_elo) : '<i>N/A</i>')."</td></tr>\n";
                echo "<tr><td>".$lng->getTrn('common/WLD')."</td><td>$this->mv_won/$this->mv_lost/$this->mv_draw</td></tr>\n";
                echo "<tr><td>".$lng->getTrn('common/WLD')." ".$lng->getTrn('common/streaks')."</td><td>$this->mv_swon/$this->mv_slost/$this->mv_sdraw</td></tr>\n";
                ...
                $result = mysql_query("
                    ...
                    FROM ".$db_prefix."teams WHERE owned_by_coach_id = $this->coach_id");
        ...
        <div class="boxCoachPage">
            ...
            <div class='boxBody'>
                ...
                $stats = array(
                    ...
                    'Tués'  => array('ki', 2),
                    'Graves'  => array('si', 2),
                    'Touchdowns'  => array('td', 2),
                    'Interceptions' => array('intcpt', 2),
                    'Passes'  => array('cp', 2),
                    'Touchdowns marqués'  => array('gf', 2),
                    'Touchdowns encaissés'  => array('ga', 2),
                    'Sportivité' => array('smp', 2),                
                );
                $thisAVG = clone $this;
