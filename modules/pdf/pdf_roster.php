...
$pdf->Cell(60, 20, "Roster:", 0, 0, 'R', false, '');
$pdf->Cell(70, 20, ($team->f_rname), 0, 0, 'L', false, '');
$pdf->Cell(300, 20, ("Coach: " . utf8_decode($team->f_cname)), 0, 0, 'R', false, '');
...
$pdf->Cell(18, $h, 'M', 1, 0, 'C', true, '');
$pdf->Cell(18, $h, 'F', 1, 0, 'C', true, '');
$pdf->Cell(18, $h, 'Ag', 1, 0, 'C', true, '');
$pdf->Cell(18, $h, 'Ar', 1, 0, 'C', true, '');
$pdf->Cell(329, $h, 'Compétences et blessures', 1, 0, 'L', true, '');
$pdf->Cell(23, $h, 'BLES', 1, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Pas', 1, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Tds', 1, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Int', 1, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Sor', 1, 0, 'C', true, '');
$pdf->Cell(23, $h, 'JPV', 1, 0, 'C', true, '');
$pdf->Cell(25, $h, 'XP', 1, 0, 'C', true, '');
$pdf->Cell(41, $h, 'Valeur', 1, 0, 'C', true, '');
...
$pdf->print_box($currentx, $currenty, 172, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', false, 'R', 'Nombre total de joueur pour le prochain match:');
...
$pdf->print_box($currentx, $currenty, 60, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', false, 'R', 'Total: (Les joueurs absents sont exclus):');
...
$h=14;
$pdf->SetFont('Tahoma', 'B', 8);
$pdf->Cell(97+75, $h, 'Champions et mercenaires', 0, 0, 'L', true, '');
$pdf->Cell(18, $h, 'M', 0, 0, 'C', true, '');
$pdf->Cell(18, $h, 'F', 0, 0, 'C', true, '');
$pdf->Cell(18, $h, 'Ag', 0, 0, 'C', true, '');
$pdf->Cell(18, $h, 'Ar', 0, 0, 'C', true, '');
$pdf->Cell(329, $h, 'Compétences', 0, 0, 'L', true, '');
//$pdf->Cell(23, $h, 'MNG', 1, 0, 'C', true, ''); // No MNG stars/mercs. They heal. ;-)
$pdf->Cell(21, $h, 'Pas', 0, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Td', 0, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Int', 0, 0, 'C', true, '');
$pdf->Cell(21, $h, 'Sor', 0, 0, 'C', true, '');
$pdf->Cell(23, $h, 'JPV', 0, 0, 'C', true, '');
$pdf->Cell(25, $h, 'XP', 0, 0, 'C', true, '');
$pdf->Cell(41, $h, 'Valeur', 0, 0, 'R', true, '');
$currenty+=14;
$pdf->SetXY($currentx, $currenty);
$h=13;
...
if ($_POST) {
  ...
  foreach ($_POST as $postkey => $postvalue) {
    ...
    if ($postvalue == "-Aucune compétence supplémentaire-") continue;
  ...
  while (list($key, $val) = each($postvars)) {
    ...
    elseif ($key == 'Corruption') { $ind_bribes = (int) $val; continue; }
    elseif ($key == 'Carte') { $ind_card = (int) str_replace('k','000',$val); continue; }
    elseif ($key == 'Relance supplémentaire') { $ind_rr = (int) $val; continue; }
    elseif ($key == 'Cuistot Halfling') { $ind_chef = (int) $val; continue; }
    elseif ($key == 'Igor') { $ind_igor = (int) $val; continue; }
    elseif ($key == 'Apothicaire local') { $ind_apo = (int) $val; continue; }
    elseif ($key == 'Sorcier') { $ind_wiz = (int) $val; continue; }
  }
...
$pdf->print_box($currentx, $currenty, 170, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', 'Primes de match ');
$pdf->print_box(($currentx += 170), $currenty, 120, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', false, 'L', '(pour le prochain match)');
$pdf->print_box(($currentx = 630), $currenty, 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', 'Biens d\'équipe'); // 156 to margin

// Checking if Wandering Apothecary should be replaced with Igor
$r=$team->f_rname;
if (($r == 'Pourris de Nurgle') || ($r == 'Khemri') || ($r == 'Necromantiques') || ($r == 'Morts-vivants')) {
  $apo_igor = 'Igor (0-1):';
  unset($inducements['Apothicaire local']);
  if (isset($ind_igor)) { 
  ...
  $apo_igor = 'Apothicaire local (0-2):';
  unset($inducements['Igor']);
  if (isset($ind_apo)) { 
    $ind_apo_igor_cost = $inducements['Apothicaire local']['cost'];
    $ind_cost += $ind_apo*$ind_apo_igor_cost; 
    $ind_apo_igor = $ind_apo;
  }
  else { $ind_apo_igor = '__'; $ind_apo_igor_cost = $inducements['Apothicaire local']['cost']; }
}
// Checking LRB6 cheaper Chef for Halfling
if (($r == 'Halfling')) $inducements['Cuistot Halfling']['cost'] = 50000;
$chef_cost = $inducements['Cuistot Halfling']['cost'];
// Checking LRB6 cheaper bribes for Goblin
if (($r == 'Goblin')) $inducements['Corruption']['cost'] = 50000;
$bribe_cost = $inducements['Corruption']['cost'];

if (isset($ind_babes)) { $ind_cost += $ind_babes*$inducements['Bloodweiser Babes']['cost']; }
else $ind_babes = '__';
if (isset($ind_bribes)) { $ind_cost += $ind_bribes*$inducements['Corruption']['cost']; }
else $ind_bribes = '__';
if (isset($ind_card)) { $ind_cost += $ind_card; }
else $ind_card = '__';
if (isset($ind_rr)) { $ind_cost += $ind_rr*$inducements['Relance supplémentaire']['cost']; }
else $ind_rr = '__';
if (isset($ind_chef)) { $ind_cost += $ind_chef*$inducements['Cuistot Halfling']['cost']; }
else $ind_chef = '__';
if (isset($ind_wiz)) { $ind_cost += $ind_wiz*$inducements['Sorcier']['cost']; }
else $ind_wiz = '__';
...
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Corruption (0-3):', $ind_bribes, $pdf->Mf($bribe_cost));
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Relance supplémentaire (0-4):', $ind_rr, $pdf->Mf($inducements['Relance supplémentaire']['cost']));
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Cuistot Halfling (0-1):', $ind_chef, $pdf->Mf($chef_cost));
...
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Sorcier (0-1):', $ind_wiz, $pdf->Mf($inducements['Sorcier']['cost']));
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Budget Cartes:', ' ', $pdf->Mf($ind_card));
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Entrées:', null, '');
...
$pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Relances:', ($team->rerolls), $pdf->Mf($rerollcost), $pdf->Mf($team->rerolls * $rerollcost), false);
$pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Popularité:', ($team->rg_ff), $pdf->Mf($rules['cost_fan_factor']), $pdf->Mf($team->rg_ff * $rules['cost_fan_factor']), false);
$pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Assistants:', ($team->ass_coaches), $pdf->Mf($rules['cost_ass_coaches']), $pdf->Mf($team->ass_coaches * $rules['cost_ass_coaches']), false);
$pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Pom-Pom Girls:', ($team->cheerleaders), $pdf->Mf($rules['cost_cheerleaders']), $pdf->Mf($team->cheerleaders * $rules['cost_cheerleaders']), false);
if ($r == 'Morts-vivants' || $r == 'Necromantiques') // Swap Apothecary for Necromancer
  $pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Necromancien:', 1, 0, 0, false);
elseif ($r == 'Pourris de Nurgle' || $r == 'Khemri')  // Remove Apothecary
  $currenty+=$h;
else  // Normal case
  $pdf->print_team_goods($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 'Apothicaire:', ($team->apothecary), $pdf->Mf($rules['cost_apothecary']), $pdf->Mf($team->apothecary * $rules['cost_apothecary']), false);
$pdf->print_box($currentx+=70, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', false, 'R', 'Trésor:' );
...
$h=13;
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', 'Valeur totale d\'équipe:');
...
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', 'Primes de match:');
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', $pdf->Mf($ind_cost));
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Tahoma', true, 'R', 'Valeur pour le match:');
...
$pdf->Cell(20, 8, 'Rate un match (RUM)', 0, 0, 'L', false);
$pdf->SetFillColorBB($pdf->hex2cmyk(COLOR_ROSTER_JOURNEY));
$pdf->Rect($currentx+=80+5, $currenty+=1, 5, 5, 'DF');
$pdf->SetXY($currentx+=5, $currenty-=1);
$pdf->Cell(80, 8, 'Franc-joueur', 0, 0, 'L', false);
$pdf->SetFillColorBB($pdf->hex2cmyk(COLOR_ROSTER_NEWSKILL));
$pdf->Rect($currentx+=50+5, $currenty+=1, 5, 5, 'DF');
$pdf->SetXY($currentx+=5, $currenty-=1);
$pdf->Cell(50, 8, 'Nouvelle compétence disponible', 0, 0, 'L', false);
$pdf->SetFillColorBB($pdf->hex2cmyk(COLOR_ROSTER_CHR_GTP1));
$pdf->Rect($currentx+=120+5, $currenty+=1, 5, 5, 'DF');
$pdf->SetXY($currentx+=5, $currenty-=1);
$pdf->Cell(120, 8, 'Augmentation de caractéristique', 0, 0, 'L', false);
$pdf->SetFillColorBB($pdf->hex2cmyk(COLOR_ROSTER_CHR_LTM1));
$pdf->Rect($currentx+=120+5, $currenty+=1, 5, 5, 'DF');
$pdf->SetXY($currentx+=5, $currenty-=1);
$pdf->Cell(120, 8, 'Diminution de caractéristique', 0, 0, 'L', false);
