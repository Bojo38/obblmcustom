...
{
    $result = mysql_query("SELECT * FROM ".$db_prefix."prizes WHERE prize_id = $prid");
...
{
    return (mysql_query("DELETE FROM ".$db_prefix."prizes WHERE prize_id = $this->prize_id"));
}

public function edit($type, $tid, $trid, $title, $txt)
{
	global $db_prefix;
    if (mysql_query("UPDATE ".$db_prefix."prizes SET 
...
public static function getPrizesByTour($trid = false, $n = false) {
    ...
    global $db_prefix;
    if (!$trid) {
        $query = "SELECT DISTINCT(prizes.tour_id) AS 'tour_id' FROM ".$db_prefix."prizes, ".$db_prefix."tours WHERE prizes.tour_id = tours.tour_id ORDER BY date_created DESC ".(($n) ? " LIMIT $n" : '');
    ...
    foreach ($tours as $t) {
        $t->prizes = array();
        $query = "SELECT prize_id, type FROM ".$db_prefix."prizes WHERE tour_id = $t->tour_id ORDER BY type ASC";
        $result = mysql_query($query);
        if ($result && mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
...
public static function getPrizesByTeam($tid)
{
    ...
    $query = "SELECT prize_id FROM ".$db_prefix."prizes WHERE team_id = $tid ORDER BY date DESC";
...
global $db_prefix;
    ...
    $query = "SELECT prize_id FROM ".$db_prefix."prizes WHERE tour_id = $trid AND type = $type";
    $result = mysql_query($query);
    if ($result && mysql_num_rows($result) > 0) {
