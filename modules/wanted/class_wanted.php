...
{
global $db_prefix;
    $result = mysql_query("SELECT * FROM ".$db_prefix."wanted WHERE wanted_id = $wanted_id");
...
{
global $db_prefix;
    if (mysql_query("UPDATE ".$db_prefix."wanted SET 
...
{
	global $db_prefix;
    return (mysql_query("DELETE FROM ".$db_prefix."wanted WHERE wanted_id = $this->wanted_id"));
}
...
global $db_prefix;
    ...
    $result = mysql_query("SELECT wanted_id, pid FROM ".$db_prefix."wanted ORDER BY date DESC" . (($n) ? " LIMIT $n" : ''));
    if ($result && mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
...
public static function getModuleUpgradeSQL()
{
    ...
    return array(
        '075-080' => array(
            'CREATE TABLE IF NOT EXISTS '.$db_prefix.'wanted
            (
                    wanted_id   MEDIUMINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT,
                    pid         MEDIUMINT UNSIGNED,
                    date        DATETIME,
            ...
            'INSERT INTO '.$db_prefix.'wanted (pid, date, why, bounty) SELECT f_id, date, txt, txt2 FROM '.$db_prefix.'texts WHERE type = 6 ORDER BY date ASC',
            'DELETE FROM '.$db_prefix.'texts WHERE type = 6',
        ),
    );
}
...
{
global $db_prefix;
    $query = "SELECT pid FROM ".$db_prefix."wanted WHERE pid = $pid";
        ...
        switch ($_GET['action'])
        {
            ...
            case 'new':
                ...
                <select name="player_id" id="players">
                    <?php
					global $db_prefix;
                    $query = "SELECT player_id, ".$db_prefix."players.name AS 'name', ".$db_prefix."teams.name AS 'team_name' FROM ".$db_prefix."players, ".$db_prefix."teams WHERE owned_by_team_id = team_id ORDER by team_name ASC, name ASC";
